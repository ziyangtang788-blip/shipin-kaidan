...
const STEPS = [
  ...
  ["测试-点位一列数量一列.js", "★★ 点位排一列、数量排另一列：按顺序配（9/6 朱先生那张的形状）"],
  ["测试-照片单走全链路.js", "★★ 照片真单走【整条真链路】：老员工该让开就让开，逐段核对（9/9 补的那个洞）"],
  ["测试-碎片类.js", "一格切碎成几类：不猜格式、角色人教一次、整列都会"],
  ["测试-学分法.js", "★ 一格分成几份：账能证明的不问、没得选的不问、分不出来不硬凑"],
  ["测试-新建品要有编号.js", "★ 新建的品要有观麦编号：自己编的假码不许填进商品ID"],
  ["测试-观麦码就地改.js", "★ 观麦商品ID 在客户页就地改：账本按原始码记、学过的对照跟着搬（9/7）"],
  ["测试-加客户和导报价单.js", "★★ 直接加客户 + 拖报价单进来：先摆出来，点了才入库（9/7）"],
  ["测试-删掉整个客户.js", "★★ 整个客户删掉：不许动 DATA.custs（动了 6000 条价格全串家）；删错能恢复（9/7）"],
  ["测试-删掉的品不许再出现.js", "★ 删掉的品不进索引：搜不到、也配不上（9/7 老板：停售的直接删掉）"],
