...
(function(root){
  ...
  /* ===== 客户报斤数时，按斤卖还是按板卖 =====
     ...
     例（裕丰 水豆腐：5斤板 ¥5.5、6斤板 ¥6.5、按斤 ¥1.5）：
       18斤 → 18÷6=3   除得尽 → 6斤板 3板
       15斤 → 15÷5=3   除得尽 → 5斤板 3板（6斤除不尽，换过来）
       17斤 → 都除不尽        → 按斤价 17斤
     例（蔬源 客家豆腐：7斤板、5斤板、按斤 ¥2）：
       18斤 → 18÷7、18÷5 都除不尽 → 按斤价 ¥36  ✓ 跟观麦一致

     两处交给人，不许替他拿主意：
       · 好几种板都除得尽（30斤：5斤板6块、6斤板5块）
       · 除不尽、这家又没有论斤卖的同款
     替他挑一个，要么少发要么多发，钱和货都不对。 */
  /* 散称的定义在 引擎-解析.js，这里只借用 —— 抄第二份就会有一天两边对不上 */
  var 散称=P.散称;
  /* 比对用的词根：把名字尾巴上的包装单位去掉，
     「水豆腐7斤/板」剥完是「水豆腐板」，要再去掉那个「板」才认得出
     它跟「水豆腐（斤）」是同一个货。 */
  function 根(s){ return bare(s).replace(/[板盒件包箱桶袋]+$/,""); }

  function 斤怎么算(DATA,b,unit,qty,i,text){
    var u0=unitNorm(unit||"");
    if(!u0||!散称.test(u0)) return null;      /* 客户写的是板/件 → 直接按板，这条不管 */
    if(i<0||!b||!(qty>0)) return null;
    /* ⚠ 配到的本来就是论斤卖的那条 → 一步都不许动。
       客户写「水豆腐（斤）5斤」「散装攸县香干45斤」「香干35斤」，
       词里就写着要散称的，去凑板是自作主张：
         45 ÷ 5 = 9 刚好整除，就把散装香干开成 9 包 —— 观麦开的是 45 斤散装。
       这一条不加，9758 行真单里凭空多出 60 多条认错。 */
    if(散称.test(unitNorm(DATA.items[i][3]||""))) return null;
    /* 「同一个货的一家人」得从两头找：
         · 配到那条的词根 —— 尝元小板豆腐（5斤）→「尝元小板豆腐」
         · 客户写的词根   —— 水豆腐（5斤装）  →「水豆腐」
       只认前者的话，碧源那种「客户叫水豆腐、库里叫尝元小板豆腐」的
       就找不到论斤卖的同款，这条规矩等于没生效。 */
    var gs=[根(DATA.items[i][2]), 根(text||"")].filter(function(x,k,a){
      return x&&x.length>=2&&a.indexOf(x)===k;
    });
    if(!gs.length) return null;
    var 家=b.list.filter(function(x){
      var gx=根(DATA.items[x][2]);
      return gx&&gs.some(function(g){ return gx===g||gx.indexOf(g)>=0||g.indexOf(gx)>=0; });
    });
    if(家.indexOf(i)<0) 家.push(i);

    /* 除得尽的板型有哪些 */
    var 整=家.filter(function(x){
      var c=换算比(DATA,x,u0);
      if(!c||!(c.mul>0)||c.mul>=1) return false;   /* 只看「散称 → 整份」这一头 */
      var v=qty*c.mul;
      return Math.abs(v-Math.round(v))<0.000001&&Math.round(v)>0;
    });
    if(整.length){
      if(整.indexOf(i)>=0) return {i:i,how:"fitboard",cands:整};   /* 配到的自己就除得尽 */
      if(整.length===1)    return {i:整[0],how:"fitboard",cands:整};
      return {i:整[0],how:"fuzzy",cands:整};                        /* 好几种都行 → 交给人 */
    }

    /* 一种板都除不尽 → 按单斤卖 */
    var 斤货=家.filter(function(x){ return 散称.test(unitNorm(DATA.items[x][3]||"")); });
    /* 跟客户写的词最像的排前面 —— 碧源写「水豆腐(5斤装)」，
       默认该是「水豆腐（斤）」，不是「九龙水豆腐（斤）」 */
    var gt=根(text||"");
    if(gt) 斤货.sort(function(x,y){
      return (根(DATA.items[y][2])===gt?1:0)-(根(DATA.items[x][2])===gt?1:0);
    });
    if(斤货.indexOf(i)>=0) return null;                             /* 配到的本来就论斤，不用动 */
    if(斤货.length===1) return {i:斤货[0],how:"byweight",cands:斤货};
    if(斤货.length>1)   return {i:斤货[0],how:"fuzzy",cands:斤货};
    return null;                                                    /* 这家没有论斤卖的，维持原样 */
  }
  ...
  function 单位对不上(DATA,b,unit,i){
    ...
    if(换算比(DATA,i,u0)) return null;                 /* 能换算，交给换算那条 */
  ...
  function 认一遍(C,ci,text,unit,priceHint,qtyHint){
    ...
    if(r&&r.i>=0){
      ...
      if(!(教U!==null&&教U===C.DATA.items[r.i][6])){
        ...
        if(同){
          ...
          r=服 ? {i:同[0],how:"byunit",cands:同}
               : {i:同[0],how:"fuzzy",cands:同,单位不符:true};
                ...
                &&!换算比(C.DATA,r.i,unit)&&commits(r.how)){
          /* 同单位的一条都没有、也换算不出 —— 不敢拍板。
             闷着按「2板当2斤」算，一板的钱就没了。
             候选里单位对得上的排前面，回车就是对的。 */
    ...
    /* 最后一道，也是最要紧的一道：客户报的是斤数还是板数（老板定稿的规矩）。
       放在最后 —— 前面几道把「是哪个货」定下来，这一道只管「按斤还是按板」。 */
    if(r&&r.i>=0&&qtyHint>0){
      var 斤板=斤怎么算(C.DATA,C.IDX.byCust[ci],unit,qtyHint,r.i,text);
      if(斤板){
        var cs2=(斤板.cands||[斤板.i]).concat(r.cands||[])
                 .filter(function(x,ix,a){ return a.indexOf(x)===ix; });
        r={i:斤板.i,how:斤板.how,cands:cs2};
      }
    }
    return r;
  }
