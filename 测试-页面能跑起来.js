...
/* ══════ 确定订单 ══════ */
{
  ...
  ok("没确认客户不给确定订单", /确定订单[\s\S]{0,400}ST\.custOK[\s\S]{0,60}alert/.test(src));
   ...
   同一个词二十几行，一行行点是活受罪。 */
{
  ...
  ok("默认是全部改（确定=全改）", /点确定 = 全部改成/.test(src));
  ...
  ok("还留着「只改这一行」这条路", /点取消 = 只改这一行/.test(src));
   ...
   只告诉人「你自己去手动加一行」不算数 —— 数量、单位、点位都得先填好。 */
{
  ...
  ok("「别的货」那条有「＋ 加进来」按钮", /addbuf/.test(src) && /加进来，我挑商品/.test(src));
  ...
  ok("按钮接上了 addBlankLine", /addbuf[\s\S]{0,400}addBlankLine\(\{/.test(src));
  ...
  ok("加进来之后记一笔", /已处理\[x\.键\]=1/.test(src));
  ...
  ok("还能「不用管」（不加进单也别再提醒）", /skipbuf/.test(src));
   ...
   猜一个数才是错 —— 猜出来的没人查得出来；写 ? 摆在明面上，填一下就完事。 */
{
  ...
  ok("提醒是从当前行现算的，不是识别时的快照",
    ...
    "拿快照的话填一个也不会少一条");
  /* 闸门 */
  ok("空着不许确定订单", /还有 "\+空着\.length\+" 处识别没看清/.test(src));
  ok("空着不许导观麦", /还有 "\+空着2\.length\+" 处识别没看清/.test(src));
}
   ...
   　　　　　　　　　所有东西都不要有猜的成分。」 */
{
  ...
  ok("人改过原文就以文字为准",
    /同一份\(text,ST\.结构文字\)/.test(src),
    "原文框跟结构对不上还硬用结构，就是拿一份过时的东西开单");
...
/* ══════ 存单/打印/调历史单：出错要说出来 ★（2026-08-11）══════ */
{
  ...
  ok("★ 确定订单那条链接住了错（不然人不知道单存没存进去）",
    ...
    "确定订单 没有 .catch");
  ok("★ 打印/导出前存单那条也接住了", /存单这一步出错了，所以没给你打印/.test(src));
