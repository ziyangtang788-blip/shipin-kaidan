...
/* ★ 2026-08-11：以前只加载 6 个，认表/读结构/断句规矩【一个都没加载】——
   ...
   ★★ 2026-08-28：8/11 补完之后【又脱节了】—— 手抄的名单停在 10 个，
   真页面已经 20 个。碎片类、学分法、按内容认单、抓取、点位册、读文件、OCR
   这七个一个都没装，于是那 294 项「页面跑得起来」，
   测的是一个【缺了一半零件的页面】。
   8/28 定位轩宝那次事故，我拿它复现，复现出来的结果是错的，白往错方向猜了一轮。

   ⛔ 根子是【手抄】。名单一旦要人手动跟，迟早跟丢 —— 这次跟丢了 10 个。
   改成直接从页面的 script 标签里读：以后加引擎自动跟上，永远脱不了节。
   密钥-本机.js 线上是 404（故意不传），这里照样跳过。 */
const 依次加载 = [...H.matchAll(/<script\s+src="([^"?]+)/g)]
  .map(m => m[1]).filter(f => f !== "密钥-本机.js");
ok("★ 加载名单是从页面读出来的，不是手抄的（零件一个不少）",
  依次加载.length >= 20, "只读到 " + 依次加载.length + " 个 —— 名单又被改回手抄了？");
let 加载出错 = null;
try {
   ...
   识别完把原图和读出来的字再发一次，只问哪个数字跟图上对不上。 */
{
  ...
  /* ★ 2026-08-21 提示词搬进 引擎-读结构.js 了 —— 页面、批量工具共用同一份，
     页面这边只准从引擎拿，不许再写死一份（写死了就会「测试全绿、页面上没改」）。 */
  ok("★ 提示词从引擎拿，页面没抄第二份",
    /核对提示\s*=\s*\(window\.GM_STRUCT/.test(src) && !/只挑【数字和单位】的出入/.test(src));
  /* 老板把这一遍的活加宽了：原来只核数字和单位，现在【数量、点位、漏没漏段】三样都要核
     —— 点位整个丢了那次它一声没吭。⚠ 守的是意图：三样都得点到，字怎么写不管。 */
  {
    const 引擎 = require("fs").readFileSync(require("path").join(__dirname, "引擎-读结构.js"), "utf8");
    ok("核对要核数量、点位、漏没漏段三样",
      /数量和单位/.test(引擎) && /点位/.test(引擎) && /漏没漏段/.test(引擎));
  }
  ...
  /* ★ 2026-08-21：核对用的图从「发出去的那坨 content」改成【原图 imgs】。
     OCR 走通那条路上 content 里全是文字块、一张图都没有 ——
     拿它去核等于让 AI 闭着眼睛看图。放大那一步不变。 */
  ok("核对用的是原图（放大版）", /核对用的图=imgs\.map/.test(src) &&
    /放大图\(图们,function\(大图\)/.test(src) && /核对一遍\(大图/.test(src));
  ...
  /* ⚠ 守意图不守写法：网络挂了要接住，交回去一个「没核成」，绝不能把识别拖死。
     ★ 2026-08-21 起交回去的是 null（没核成），【不是空数组】——
       空数组会被当成「核过了、全对」，等于替 AI 说了句它没说过的话，
       单子照错不误，页面还显示绿的。 */
  ok("核对失败不吭声（不能拖累识别）",
    /function 核对一遍[\s\S]{0,2600}catch\(function\(\)\{\s*收到\(null\);\s*\}\)/.test(src),
    "核对是加分项，失败了不能反过来把识别搞挂");
  ok("★ 没核成 ≠ 全对（不许拿空当全对）", /没核成/.test(src));
  ...
  /* ★ 2026-08-04 老板定的三件（都在这一批改的，钉住免得被改回去）：
       ① 界面分上下两块：先摆要拿主意的，再列没问题的　← 2026-08-20 被他自己推翻，见下
       ② 粘贴要收【文件】不只是图片（「是文件收不到，不是图片」）
       ③ 从微信拖进不来时必须说一句，不许闷着 */
  /* ⚠ 2026-08-20 老板推翻了 ①：「我想全部按照我给系统的单的顺序出。」
     理由是分两块把客户单子的行序切断了，打印出来的单也跟着乱（打印印的就是屏幕）。
     所以这三条从「必须分块」反过来钉成「不许分块」。
     怎么找问题行改为靠颜色（琥珀=要填、红=对不上、青=你动过），
     完整的一份在 测试-出单按原顺序.js。 */
  ok("★ 不许再把行分堆重排（老板 8/20：按客户单子的原顺序）",
    !/var 要=待排\.filter/.test(src) && !/待排\.push/.test(src));
  ok("★ 行是照 R.rows 原顺序直接贴的", /R\.rows\.forEach\(function\(r,n\)/.test(src));
  ok("★ 折叠条跟着去掉了（它是靠分组实现的）",
    !/rowsplit/.test(src) && !/okrow/.test(src));
  ...
  ok("清空单据时把核对结果也清掉", /核对结果=null; 核对中=false;/.test(src));
   ...
   　　　　　　　　　所有东西都不要有猜的成分。」 */
{
  ...
  /* ⚠ 守的是【失败了文件内容要放回原文框】，不是那个函数的参数长什么样。
     原来写的是 /function 识别败了\(话\)/ —— 2026-08-21 给它加了第二个参数
     （交叉表不许硬读）就误报了一次。守写法不守意图，改一次挂一片。 */
  ok("识别失败也把文件内容放回原文框",
    /function 识别败了\(/.test(src) &&
    /识别败了[\s\S]{0,600}?\$\("raw"\)\.value=\(老\?老\+"\\n\\n":""\)\+新文/.test(src),
...
/* ══════ 真走一遍「传表 → 认表 → 读法本」★（2026-08-11）══════
   ...
   ★★ 2026-08-28：零件装齐之后，这一整段【全红了】——
   真页面先跑「按长相直接抓」那条近道（引擎-抓取.js），裕丰这张规整表它一把就抓出
   3 样货，一次 AI 都不问，压根不进认表、也不进读法本。
   以前零件缺了 10 个、近道那个引擎根本没装，才会掉进认表路 ——
   所以这一段十几年如一日的绿，测的是【一个装不全零件的假页面才有的行为】。

   ⚠ 不是把断言改掉迁就现状：认表+读法本这条路真实存在，近道抓不动时就走它，
     必须有人守着。要测到它，就得喂一张【近道会主动罢手】的表。
     判据用代码里明写的那条：一份文件里两张工作表都出货 → 用哪张定不下来
     → 近道自己 return null，交回认表路。（引擎里的原话：「不许自己挑」）
   第 2 张只是用来逼近道罢手；读法说的是「用第 1 张」，货照旧从第一张摊。 */
const 裕丰份 = [{ name: "裕丰.png", grid: [
  { n: 1, rows: [
    ["大豆泡", "", "斤", "7", "218"],
    ["水豆腐", "5斤/板", "板", "3", "218"],
    ["千张", "", "斤", "3", "106"]] },
  { n: 2, rows: [
    ["大豆泡", "", "斤", "7", "218"],
    ["水豆腐", "5斤/板", "板", "3", "218"]] }] }];
/* 单表那张留着：钉住「规整表走近道、不花钱」这个真页面的真实行为 */
const 裕丰单表份 = [{ name: "裕丰单表.png", grid: [{ n: 1, rows: [
  ["大豆泡", "", "斤", "7", "218"],
  ["水豆腐", "5斤/板", "板", "3", "218"],
  ["千张", "", "斤", "3", "106"]] }] }];
...
async function 跑读法本() {
  ...
  /* ── ★★ 2026-08-28 补：规整的单表走【近道】，一个 token 都不花 ──
     零件装齐之前，近道那个引擎根本没装进测试页面，这条路【一次都没被测到过】。
     可 8/28 轩宝那次事故就出在这条路上（掉进近道、把整格当一个点位、账还是平的），
     它是真页面最常走的一条，必须有人守着。 */
  T.拿OV().读法本 = {};
  let 近道问了几次 = 0;
  win.fetch = () => { 近道问了几次++; return Promise.reject(new Error("走近道不该问 AI")); };
  EL["raw"].value = "";
  EL["btn-ocr"].disabled = true;
  let 近道出错 = null;
  try { T.认表跑一趟(裕丰单表份, "假密钥"); } catch (e) { 近道出错 = e; }
  await 等一轮();
  ok("★ 规整的单表走近道，不许抛", !近道出错, 近道出错 && 近道出错.message);
  ok("★ 走近道一次 AI 都没问（省的就是这笔钱）", 近道问了几次 === 0,
    "问了 " + 近道问了几次 + " 次");
  ok("★ 走近道货照样摊出来了", /大豆泡/.test(EL["raw"].value || ""),
    JSON.stringify((EL["raw"].value || "").slice(0, 120)));
  ok("★ 屏幕上说明白了这趟没花钱",
    /没花钱/.test(EL["ocr-msg"].innerHTML || ""),
    (EL["ocr-msg"].innerHTML || "").slice(0, 240));
}
